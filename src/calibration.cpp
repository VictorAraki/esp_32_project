...
// =====================================================
// Calibração diagnóstica de 1 ou 2 MPU6050 (DEBUG)
// Versão: 0.2.1 - com diagnostico detalhado
//
// Mostra exatamente qual step falha
// =====================================================
...
// ================================
// Calibração com diagnostico
// ================================
bool collectAverages(uint8_t addr, SensorCalibResult &res) {
  if (!i2cResponds(addr)) {
    res.failureReason = "I2C nao responde no endereco";
    return false;
  }

  uint8_t who = 0;
  if (!readRegister8(addr, REG_WHO_AM_I, who)) {
    res.failureReason = "Nao conseguiu ler WHO_AM_I";
    return false;
  }
  if (who != 0x68) {
    res.failureReason = "WHO_AM_I retornou valor errado";
    Serial.print("  (esperado 0x68, recebido 0x");
  ...
  if (!initMPU6050(addr)) {
    res.failureReason = "Falha na inicializacao (power/config/WHO_AM_I)";
    return false;
  }
  ...
  for (int i = 0; i < NUM_SAMPLES; i++) {
    ...
    if (!readMPU14(addr, ax, ay, az, tempRaw, gx, gy, gz)) {
      res.failureReason = "Falha lendo dados durante amostragem";
      Serial.print("  (falhou na amostra ");
      Serial.print(i);
      Serial.println(")");
      return false;
    }
...
void printResult(const char *label, uint8_t addr, const SensorCalibResult &r) {
  ...
  if (!r.ok) {
    ...
    Serial.print("Motivo: ");
    Serial.println(r.failureReason);
    Serial.println("========================================");
    return;
  }
...
void setup() {
  ...
  Serial.println();
  Serial.println("=== CALIBRACAO DIAGNOSTICA DEBUG - 1 ou 2x MPU6050 ===");
